...
if (isset($_SESSION['edit_gallery_err']) && isset($_SESSION['edit_gallery_color'])) {
    $edit_gallery_err = $_SESSION['edit_gallery_err'];
    $edit_gallery_color = $_SESSION['edit_gallery_color'];
    unset($_SESSION['edit_gallery_err']);
    unset($_SESSION['edit_gallery_color']);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['edit_gallery'])) {
        $gallery_id = intval($_POST['edit_gallery_id']);
        $new_title = trim($_POST['edit_gallery_title']);
        $new_description = trim($_POST['edit_gallery_desc']);
    
        if (empty($new_title)) {
            $_SESSION['edit_gallery_err'] = "Název galerie nesmí být prázdný";
            $_SESSION['edit_gallery_color'] = "red";
            header("Location: admin?side=gallery");
            exit;
        }
    
        $sql = "UPDATE gallery SET title = ?, description_gal = ? WHERE gallery_id = ?";
        $stmt = $conn->prepare($sql);
    
        if ($stmt) {
            $stmt->bind_param("ssi", $new_title, $new_description, $gallery_id);
    
            if ($stmt->execute()) {
                $_SESSION['edit_gallery_err'] = "Galerie byla úspěšně upravena.";
                $_SESSION['edit_gallery_color'] = "green";
            } else {
                $_SESSION['edit_gallery_err'] = "Chyba při úpravě galerie.";
                $_SESSION['edit_gallery_color'] = "red";
            }
    
            $stmt->close();
        } else {
            $_SESSION['edit_gallery_err'] = "Chyba při přípravě dotazu.";
            $_SESSION['edit_gallery_color'] = "red";
        }
...
<body>
    ...
    <main>
        ...
        <div class="center switchable switchedOff"> <!-- gallery -->
            <div class="left">
                <div class="vertical">
                    <form id="add_images_form" class="form_green" action="admin" method="POST" autocomplete="off" enctype="multipart/form-data">
                        ...
                        <select id="gallery_in" name="gallery">
                    ...
                    <form id="edit_gallery_form" class="form_green" action="admin" method="POST" autocomplete="off">
                        <h2>Upravit Galerii</h2>
                        <label for="edit_gallery_in">Vyberte galerii</label>
                        <select id="edit_gallery_in" name="edit_gallery_id">
                            <?php GetAllGaleriesOption($conn); ?>
                        </select>

                        <label for="edit_gallery_title_in">Nový název galerie</label>
                        <input type="text" name="edit_gallery_title" required id="edit_gallery_title_in" placeholder="Nový název galerie">
                        <label for="edit_gallery_desc_in">Nový popisek</label>
                        <input type="text" name="edit_gallery_desc" id="edit_gallery_desc_in" placeholder="Nový popisek galerie">

                        <input type="hidden" name="edit_gallery" value="true">
                        <span style="color: <?php echo htmlspecialchars($edit_gallery_color ?? ''); ?>;"><?php echo htmlspecialchars($edit_gallery_err ?? ''); ?></span>
                    ...
                    <form id="remove_gallery_form" class="form_green" action="admin" method="POST" autocomplete="off">
                        ...
                        <select id="gallery_out" name="gallery_out">
                            <?php GetAllGaleriesOption($conn) ?>
                        </select>
        ...
        <div class="blockCenter switchable center" id="cardsMenu"> <!-- cards -->
            <div class="left">
                <form id="add_card_form" class="form_green" action="admin" method="POST" autocomplete="off" enctype="multipart/form-data">
                    ...
                    <textarea id="text_in" name="card_text" style="height: 5rem;" placeholder="Text pod novým příspěvkem"><?php echo htmlspecialchars($card_text_old) ?></textarea>
